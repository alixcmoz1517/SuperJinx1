...
# راه‌اندازی و اتصال به دیتابیس SQLite برای ذخیره دائمی اطلاعات
def init_db():
...
def get_user(user_id):
  ...
  cursor = conn.cursor()
  cursor.execute(
      'SELECT user_id, balance, refs, wallet, invited_by FROM users WHERE'
      ' user_id = ?',
      (user_id,),
  )
  row = cursor.fetchone()
...
def update_user_balance_refs(referrer_id, new_balance, new_refs, referred_id):
  ...
  cursor = conn.cursor()
  cursor.execute(
      'UPDATE users SET balance = ?, refs = ? WHERE user_id = ?',
      (new_balance, new_refs, referrer_id),
  )
  cursor.execute(
  ...
  cursor.execute(
      'INSERT OR IGNORE INTO referred_users (referrer_id, referred_id) VALUES'
      ' (?, ?)',
      (referrer_id, referred_id),
  )
  conn.commit()
...
def is_already_referred(referrer_id, referred_id):
  ...
  cursor = conn.cursor()
  cursor.execute(
      'SELECT * FROM referred_users WHERE referrer_id = ? AND referred_id = ?',
      (referrer_id, referred_id),
  )
  row = cursor.fetchone()
  conn.close()
...
def send_welcome(message):
  try:
    ...
    if not check_membership(user_id):
      ...
      bot.send_message(
          message.chat.id, stylish_text, reply_markup=forced_sub_markup()
      )
      return
  ...
  if is_new_user:
    ...
    try:
      join_text = (
          ...
          f'👤 نام: {first_name}\n'
          f'🆔 آیدی عددی: `{user_id}`\n'
          f'🔗 اکانت: {user_display_name}'
      )
      bot.send_message(CHANNEL_USERNAME, join_text, parse_mode='Markdown')
    except Exception as e:
      print(f'Error sending join log to channel: {e}')
  ...
  if len(args) > 1 and is_new_user:
    try:
      ...
      if (
          referrer_id != user_id
          and referrer_data
          and user_data['invited_by'] is None
      ):
        if not is_already_referred(referrer_id, user_id):
          ...
          try:
            notify_referrer_text = (
                f'کاربر {user_display_name} با موفقیت به زیرمجموعه شما اضافه شد'
                ' ✅ (+8$)'
            )
            bot.send_message(
                referrer_id, notify_referrer_text, parse_mode='Markdown'
            )
...
def handle_messages(message):
  try:
    ...
    if user_states.get(user_id) == 'waiting_for_bnb_wallet':
      ...
      markup.add(
          InlineKeyboardButton(
              '✅ تایید و ارسال واریزی', callback_data=f'pay_done_{user_id}'
          )
      )

      try:
        bot.send_message(
            CHANNEL_USERNAME, channel_text, reply_markup=markup, parse_mode='Markdown'
        )
      except Exception as ex:
        print(f'Error sending to channel: {ex}')

      success_withdraw_text = (
          'درخواست برداشت شما با موفقیت ثبت شد ✅\n\nآدرس ولت شما دریافت شد. پس از'
          ' بررسی و واریز مدیریت، تاییدیه به شما اعلام می‌گردد 🔜'
      )
      bot.send_message(
          message.chat.id, success_withdraw_text, reply_markup=colored_menu()
      )
      return
    ...
    if text == '👤 حساب کاربری':
      ...
      account_text = (
          '📢  با دعوت دوستانت درآمد کسب کن! \n\nدوستانت را به ربات دعوت کن و به'
          ' ازای هر نفری که دعوت می‌کنی 8.0$ پاداش بگیر و درآمد واقعی داشته باش!'
          ' 🎁 \n\n🔗  لینک اختصاصی دعوت شما:\n'
          f'{invite_link}\n\n⏳  فرصت را از دست نده! هرچه افراد بیشتری را دعوت'
          ' کنی، سریع‌تر موجودی‌ات را افزایش می‌دهی و می‌توانی آن را برداشت کنی.'
          ' ✅'
      )
      ...
      if balance < 75.0:
        ...
        bot.send_message(
            message.chat.id, '❌ برای برداشت باید حداقل ۹ نفر را دعوت کرده باشید.'
        )
      else:
      ...
      invite_text = (
          f'👤 آیدی شما : {user_id}\n💰 موجودی شما : ${balance}\n👥 تعداد'
          f' زیرمجموعه های شما : {refs}\n💸 حداقل مبلغ برداشت : 75.0$\n\nلینک'
          f' زیرمجموعه گیری شما:\n{invite_link}'
      )
      markup = InlineKeyboardMarkup()
      markup.add(
          InlineKeyboardButton(
              '📝 کپی لینک دعوت 📋', callback_data='copy_invite_link'
          )
      )
      ...
      help_text = (
          '🌐 آموزش دریافت آدرس شبکه BEP20 از تراست ولت\n\nبرای برداشت وجه، باید'
          ' آدرس کیف پول خودتون با شبکه‌ی BEP20 (BSC) رو ارسال کنید. مراحل زیر رو'
          ' دنبال کنید:\n\n۱. اپلیکیشن Trust Wallet رو باز کنید\n۲. دکمه‌ی Receive'
          ' (دریافت) رو بزنید\n۳. ارز BNB رو انتخاب کنید (شبکه‌ش به‌صورت پیش‌فرض'
          ' BEP20 هست)\n✅ آدرسی که نمایش داده می‌شه رو کپی کنید (با آیکون کپی کنار'
          ' آدرس)\n\n🔗 توجه داشته باشید آدرس شبکه‌ی BEP20 برای همه‌ی ارزها'
          ' یکسانه.\n\n❌ مهم: اگه شبکه‌ی اشتباه (مثل ERC20 یا TRC20) رو انتخاب'
          ' کنید، امکان واریز وجود نداره و ممکنه دارایی‌تون از دست بره.\n\nبعد از'
          ' کپی کردن آدرس، اون رو دقیقاً همون‌طور که کپی کردید (بدون فاصله یا'
          ' کاراکتر اضافه) برای ربات ارسال کنید.'
      )
    ...
    if data == 'check_sub':
      if check_membership(user_id):
        bot.answer_callback_query(
            call.id, 'عضویت شما با موفقیت تایید شد! ✅', show_alert=False
        )
        try:
        ...
        bot.send_message(
            call.message.chat.id, welcome_text, reply_markup=colored_menu()
        )
      else:
        bot.answer_callback_query(
      ...
      bot.answer_callback_query(
          call.id, 'تایید شد. پیام واریزی با تاخیر ۱ دقیقه به کاربر ارسال می‌شود.'
      )
      try:
      ...
      def send_success_with_delay():
        time.sleep(60)
        success_paid_text = (
            '✅ پرداخت شما با موفقیت انجام شد!\n\nمبلغ درخواستی به ولت شما واریز'
            ' گردید.'
        )
        try:
...
print(
    'High-speed Bot with SQLite Database & Anti-Cheat System is running...'
)
